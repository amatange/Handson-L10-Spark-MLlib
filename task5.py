...
spark = SparkSession.builder.appName("FareTrendPrediction").getOrCreate()
...
if not os.path.exists(MODEL_PATH):
    ...
    hist_features = hist_windowed_df.withColumn("hour_of_day", hour(col("window.start"))).withColumn("minute_of_hour", minute(col("window.start")))
...
windowed_df = parsed_stream.groupBy(window(col("event_time"), "5 minutes", "1 minute")).agg(avg("fare_amount").alias("avg_fare"))

# Apply the same feature engineering to the streaming windowed data.
windowed_features = windowed_df.withColumn("hour_of_day", hour(col("window.start"))).withColumn("minute_of_hour", minute(col("window.start")))
